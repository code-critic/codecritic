...
class Colouring :
    ...
    def __init__(self, in_stream) :
        ...
        self.neighbour_colors = np.zeros( (len(self.vertices), self.n_colors), dtype=int)
        # Table to mark how many times is every color used in neighbors.
        self.vtx_degs = np.array([len(vtx.adj) for vtx in self.vertices])
        self.max_deg = np.amax(self.vtx_degs) + 1
        self.max_deg_scale = self.max_deg * np.ones(len(self.vertices), dtype=int)
    ...
    def graph_from_stream(self, in_stream):
        ...
        for vtx in self.vertices:
            vtx.adj = np.array(list(vtx.adj), dtype=int)
    ...
    def mark_neighbors(self, i_vtx, color):
        ...
        self.neighbour_colors[adj, color] += 1
    ...
    def find_vtx_to_color(self):
        n_used_colors = np.count_nonzero(self.neighbour_colors, axis=1)
        i_vtx = np.argmax(self.max_deg_scale * n_used_colors + self.vtx_degs)
