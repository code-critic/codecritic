$(document).ready(function () {
    ...
    var registerReviewEvents = function () {
        $('.hljs-ln-n').click(function (event) {
            ...
            $tr.after(Templates.render('review/review-editor', {
                line: line
            }));
            updateCounterPlaceholder();
        });
    ...
    var renderComments = function (review) {
        if (review) {
            for (var line in review) {
                ...
                if ($line.length) {
                    $line.parent().parent().after(Templates.render('review/review-comment', {
                        review: review[line],
                        user: window.user.id,
                    }));
                }
                $('.review-comment .review-comment-entry');
    ...
    if (_id) {
        ...
        if (active.length) {
            ...
            if (first.length) {
                ...
                $('.problem-results-holder').html('<div class="white col-12 p-3"><h1>No results found</h1></div>');
            }
        }
    }
});
